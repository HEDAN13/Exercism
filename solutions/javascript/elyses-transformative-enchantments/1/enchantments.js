...
export const sandwichTrick = (deck) => {
  ...
  const lastCard = deck[len - 1];
  const middleIndex = Math.floor(len / 2) - 1;
  const middlePart = deck.slice(1, len - 1);
  return [
    ...middlePart.slice(0, middleIndex),
    lastCard,
    firstCard,
    ...middlePart.slice(middleIndex),
  ];
};
